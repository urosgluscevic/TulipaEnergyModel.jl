...
peak_demands = [6000, 7000, 8000, 9000]
...
function solve_model_reusing!(connection, inv_folder, case, dem, wind, sol, baseline_case_name)
    filepath =
        joinpath(pwd(), "$experiment_results_dir/$inv_folder/$case-investments-$dem-$wind-$sol.csv")

    if isfile(filepath)
        if case == baseline_case_name
            obj_func =
                parse(Float64, readline("$experiment_results_dir/$case-obj-$dem-$wind-$sol.txt"))
            ...
            return obj_func, investments_df
        else
            ...
            investments_made =
                get_table(connection, "var_assets_investment")[:, [:asset, :solution]]
            ...
            if case == baseline_case_name
                obj_func = energy_problem.objective_value
                open("$case-obj-$dem-$wind-$sol.txt", "w") do io
                    println(io, string(obj_func))

                    return obj_func, investments_made
                end
            else
                return 0, investments_made
            end
        end
    end

    # if it is a file, check if it is the same as the regret baseline file. If it is, read out the investments and ovj cost.
    # If it is not, read out the investments
    # if it is not a file, just solve it and read out what's needed as above
