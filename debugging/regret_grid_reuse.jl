...
experiment_inputs_dir = "debugging/experiment-inputs/regret-reuse"
experiment_results_dir = "debugging/experiment-results-regret-reuse"
...
function regret_calculation(
    ...
    input_folder = joinpath(pwd(), "$experiment_inputs_dir/investment_problem")
    ...
    input_folder_baseline = joinpath(pwd(), "$experiment_inputs_dir/operation_problem")
...
for i in 1:length(peak_demands)
    for j in 1:length(wind_limits)
        ...
        connection = input_setup_regret("$experiment_inputs_dir/investment_problem")
